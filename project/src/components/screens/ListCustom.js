...
function ListCustom(props, {navigation}) {
    ...
    const RenderItemListCustom =({item})=>{
        return(
            <TouchableOpacity Style = {{flex :1}} onPress = {()=> onChangeScreenEdit(item)}> 
                <View style = {{width: "95%", alignSelf: "center", flexDirection: "row", backgroundColor :"#dbde14",marginTop: 10, borderRadius:5, marginBottom: 10}}>
    ...
    function onChange(text){
        ...
        setInput_name(text);
        if(text){
            setDataCustom([...(dataListCustom??[]).filter(y=>y.name.toLowerCase().includes(text.toLowerCase()) || y.phone.includes(text) || y.address.toLowerCase().includes(text.toLowerCase()))])
        }
        else {
            setDataCustom(dataListCustom);
        }
