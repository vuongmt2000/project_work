...
function OverView(props) {
    ...
    useEffect(()=>{
        ...
        for(let k =0; k< lenPlace; k++){
            ...
            for(let v =0; v < lenPlace_Product; v ++){
              tong += Math.floor(
                (1 - data[k].Place_Product[v].discount / 100) * data[k].Place_Product[v].valueProduct * data[k].Place_Product[v].quantity)
            }
           tongPrice +=tong;
        }
    ...
    const RenderItem =({item}) =>{
        return (
            <TouchableOpacity onPress = {()=> changeEdit(item)}
            style = {{width: "95%", alignSelf :"center", height: 50, backgroundColor:"#acc3e8", marginTop: 5, justifyContent :"center", alignItems: "center", borderRadius:5, marginBottom:5}}>
           ...
           <View style ={{backgroundColor: "#b50000",height: 220, width:"100%", flex :1, borderBottomLeftRadius:100, justifyContent: "center", alignItems:"center"}}>
            <View style = {{height: 130, width: 300, backgroundColor:"white", borderRadius: 10}}>
                <View style ={{width:"90%", alignSelf:"center"}}>
                    ...
                    <Text style ={{alignSelf:"center"}}>{revenue .toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} Đ</Text>
                </View>
                ...
                <View style ={{}}>
                    ...
                    <Text style ={{alignSelf:"center",fontWeight: "bold"}}>{newPlace}</Text>
                </View>
                <View style ={{}}>
                    <Text style ={{alignSelf:"center", marginTop:10}}>Đơn hủy</Text>
                    <Text style ={{alignSelf:"center",fontWeight: "bold"}}>{cancelPlace}</Text>
                </View>
                <View style ={{}}>
                    <Text style ={{alignSelf:"center", marginTop:10}}>Đơn hoàn thành</Text>
                    <Text style ={{alignSelf:"center",fontWeight: "bold"}}>{donePlace}</Text>
                </View>
                </View>
            </View>
           </View>
