...
import { updateProductAction , deleteProductAction} from '../../actions/index'
function EditProduct(props) {
  const { item } = props.route.params;
  const [nameProduct, setNameProduct] = useState(item.nameProduct);
  const [valueProduct, setValueProduct] = useState(item.valueProduct.toString());
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      >
        ...
        <TouchableOpacity
        onPress ={()=> updateProduct(nameProduct, parseInt(valueProduct), imageProduct, noteProduct, item.id)}
          style={{
            height: 50,
            width: 120,
            borderRadius: 5,
